...
# =========================================================================
# MÓDULO 2: PACIENTES (CON SIMULADOR)
# =========================================================================
    ...
    if origen_datos == "Usar datos simulados de demostración":
        ...
        if archivo_cargado is not None:
            ...
            st.warning("Por favor sube un archivo CSV para continuar en este modo.")
            st.stop()

    if st.checkbox("Ver base de datos actual de Pacientes"):
        st.dataframe(df_pacientes)

    # --- PROCESAMIENTO Y AJUSTE DE ENCODERS ---
    # Creamos un diccionario de encoders para poder traducir la entrada del simulador después
    dict_encoders = {}
    ...
    for col in columnas_texto:
        if col in df_procesado.columns:
            le = LabelEncoder()
            # Aseguramos que todo sea string y entrenamos el traductor de esa columna
            df_procesado[col] = df_procesado[col].astype(str)
            le.fit(df_procesado[col])
            df_procesado[col] = le.transform(df_procesado[col])
            dict_encoders[col] = le # Guardamos el codificador entrenado
    ...
    # =========================================================================
    # NUEVA SECCIÓN: SIMULADOR INTERACTIVO DE NUEVO PACIENTE
    # =========================================================================
